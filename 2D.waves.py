...
z_array = np.linspace(-0.01, 3.0, pix_z + 1)  # spatial z grid (height) [Mm]
...
tau_val = 10000000  # 250  # Radiative Damping [s]
...
for nom in tqdm(range(0, total_nom_waves), desc=f"Computing Initial Waves"):
    ...
    rand_omega = build_functions.random_omega_value_generator(
        1.0, 0.5, (chosen_N * 1000) / 2 / np.pi, total_len=1
    )
    ...
    wave_domain += vertical_displacement
